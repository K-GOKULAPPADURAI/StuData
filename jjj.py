...
class StudentDetailsApp:
    def __init__(self, root):
        ...
        canvas = tk.Canvas(main_frame, width=800, height=600)  # Adjust width and height as needed
        

        # Load and display the background image
        bg_image = tk.PhotoImage(file="backgrounds/pic1.ppm")  # Replace with your image file path
        canvas.create_image(0, 0, image=bg_image, anchor="nw")
        canvas.pack()

        # Now you can add your buttons and other widgets on top of the Canvas as needed
        new_button = tk.Button(canvas, text="NEW", command=self.open_new_window, font=("Helvetica", 12))
        new_button.place(x=10, y=10)  # Use place() to specify the exact position

        edit_button = tk.Button(canvas, text="EDIT", command=self.open_edit_window, font=("Helvetica", 12))
        edit_button.place(x=10, y=60)

        upload_button = tk.Button(canvas, text="FILE UPLOAD", command=self.file_upload, font=("Helvetica", 12))
        upload_button.place(x=10, y=110)

        export_button = tk.Button(canvas, text="FILE EXPORT", command=self.file_export, font=("Helvetica", 12))
        export_button.place(x=10, y=160)
